#  == vs ===
1. == 会发生隐式类型转换, 所以只能判断值是否相等
2. === 不会发生类型转换, 所以会判断值和类型是否相等

# 类型转换
- 显示类型转换
    1. 转布尔 Boolean(x)
    2. 装数字 Number(x) https://es5.github.io/#x15.5.1.1
...
# valueOf()
- valueOf在对象的原型上, 他只能将包装类的对象转化为原始类型

# toString()
- js中大部分的构造函数原型上都重写了 toString 方法
1. {}.toString 返回有'[object' 和 [[class]] 和 ']' 组成的字符串
2. [].toString 返回有数组中每个元素以逗号拼接而成的字符串
